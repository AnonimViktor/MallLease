...
CREATE TABLE shopping_center (
    ...
    map_path           VARCHAR(512)  -- NULL = no interactive map for this centre
);

-- TRADE_POINT
-- status (free/occupied/unavailable) is derived by SQL CASE, not stored
...
-- one active slot per manager at a time; refused slots don't block re-booking
...
CREATE TABLE contract_rental (
    ...
    -- no two active rentals for the same point can overlap
